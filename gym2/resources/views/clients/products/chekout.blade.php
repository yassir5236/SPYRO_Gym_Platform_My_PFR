        ...
        @if (session('cart'))
            ...
            <div class="mt-8 bg-gray-900 rounded-lg p-6">
                ...
                <form action="{{ route('checkout.confirm') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="mt-4 bg-green-600 hover:bg-green-500 text-white font-semibold px-4 py-2 rounded">Confirm
                        Purchase</button>
                </form>
                <button onclick="confirmPurchase()" class="mt-4 m-5 bg-gray-500 hover:bg-gray-400 text-white font-semibold px-4 py-2 rounded">Print devis</button>
               </div>
            </div>
        @else
